...
typedef struct ms_sgxsan_ecall_dump_sancov_t {
    uint64_t ms_cntrs_start;
    uint64_t ms_cntrs_end;
    uint64_t ms_pcs_start;
    uint64_t ms_pcs_end;
} ms_sgxsan_ecall_dump_sancov_t;
...
static sgx_status_t SGX_CDECL
Enclave_sgxsan_ocall_init_shadow_memory(void *pms)
{
    ms_sgxsan_ocall_init_shadow_memory_t *ms =
        SGX_CAST(ms_sgxsan_ocall_init_shadow_memory_t *, pms);
    sgxsan_ocall_init_shadow_memory(ms->ms_enclave_base, ms->ms_enclave_size);

    return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL
Enclave_sgxsan_ocall_print_string(void *pms)
{
    ms_sgxsan_ocall_print_string_t *ms =
        SGX_CAST(ms_sgxsan_ocall_print_string_t *, pms);
...
static sgx_status_t SGX_CDECL
Enclave_sgxsan_ocall_addr2line(void *pms)
{
    ms_sgxsan_ocall_addr2line_t *ms =
        SGX_CAST(ms_sgxsan_ocall_addr2line_t *, pms);
...
static const struct {
    ...
    76,
    {
        ...
        (void *)Enclave_sgxsan_ocall_init_shadow_memory,
        (void *)Enclave_sgxsan_ocall_print_string,
        (void *)Enclave_sgxsan_ocall_addr2line,
...
sgx_status_t
sgxsan_ecall_dump_sancov(sgx_enclave_id_t eid, uint64_t cntrs_start,
                         uint64_t cntrs_end, uint64_t pcs_start,
                         uint64_t pcs_end)
{
    sgx_status_t status;
    ms_sgxsan_ecall_dump_sancov_t ms;
    ms.ms_cntrs_start = cntrs_start;
    ms.ms_cntrs_end = cntrs_end;
    ms.ms_pcs_start = pcs_start;
    ms.ms_pcs_end = pcs_end;
